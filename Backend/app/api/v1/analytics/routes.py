...
# Fecha: 14/11/2025
# Descripción: Rutas API para analytics con endpoints de exportación a CSV y PDF
#              Incluye descarga de reportes de ventas y productos
...
def get_dashboard_stats(
    ...
    - Producto top del momento
    - Resumen del día
    - Datos para gráficas (ventas mensuales, categorías, suscriptores)
    """
...
async def export_sales_report_csv(
    ...
    - Detalles diarios de ventas
    - Métricas de pedidos y valores promedio
    
    Query params:
    - **start_date**: Fecha inicial (por defecto últimos 30 días)
...
async def export_low_stock_csv(
    ...
    Genera un archivo CSV con los productos cuyo stock está por debajo
    del umbral especificado.
    
    Query params:
    - **threshold**: Umbral de stock (por defecto 10)
    ...
    writer.writerow([])
    writer.writerow(['ID', 'Nombre', 'Categoría', 'Stock Actual', 'Precio', 'Estado'])
...
async def export_product_report_pdf(
    ...
    Genera un documento PDF con tabla completa de productos y sus métricas.
